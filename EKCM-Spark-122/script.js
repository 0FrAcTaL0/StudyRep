 ...
 *    одну из двух операций, без свапа (см. pressRegisterButton). Выбор
 *    между ними определяется ДЕЙСТВИЕМ, предшествовавшим нажатию (см.
 *    state.registerMode), а не тем, что сейчас на экране:
 *      - "заслать с перезаписью" (registerMode === 'store'), если перед
 *        нажатием было: ввод числа (цифра/точка), степень, корень,
 *        вычисление целой части (ВЦ) или отрицание (/-/) - текущее
 *        значение всегда записывается в регистр, замещая собой то, что там
 *        было (даже если там уже что-то лежало) - старое содержимое
 *        регистра нигде не сохраняется и на экран не возвращается;
 *      - "вытащить с перезаписью, заменяя нулями" (registerMode === 'recall'),
 *        если перед нажатием было: +, -, ×, ÷, обратное деление, накопление
 *        в регистрах 2 и 3, СК, нажатие другой кнопки A1..A5, вывод буфера
 *        клавиатуры ("Печать") - значение регистра выводится на экран, а
 *        сам регистр очищается (становится пустым/"нулевым", как будто в
 *        него никогда не заносили значение).
 *    Операция "(" на этот выбор не влияет вовсе (registerMode не трогается);
 *    "=" и ")" в списке триггеров тоже не участвуют. Занесение в регистр
 *    может дополнительно "удалять источник" (после сохранения экран
 *    возвращается к дежурному "0") или нет - режим переключается константой
 *    MEMORY_STORE_DELETES_SOURCE.
 ...
 *    A1-A5.
 *  - если A1 задан (не null) и второй операнд не введён явно, он берётся из
 *    A1 (сам регистр не меняется, см. getCurrentTermValue) - это касается
 *    всех бинарных операций, включая обратное деление, а срабатывает эта
 *    автоподстановка строго в следующих случаях (см. AUTO_SUBSTITUTION_KEYS,
 *    state.operandPending): после нажатия одной из клавиш +,-,×,÷,обр.дел
 *    следует нажатие любой из этих же пяти клавиш (в т.ч. повторно той же -
 *    тогда недостающий операнд первого оператора берётся из A1, если он не
 *    задан - подставляется устаревшее число с экрана, т.е. фактически
 *    удваивается), либо клавиши степени, корня, ")", "=" или ВЦ. Нажатие
 *    ЛЮБОЙ другой клавиши (цифра, точка, "(", "/-/", "()", СК, регистровые
 *    кнопки) эту подстановку снимает, ничего не подставляя.
...
function createCalculator() {
  ...
  // Клавиши, участвующие в правиле автоподстановки А1 на месте пропущенного
  // операнда (см. state.operandPending, getCurrentTermValue): это пять
  // клавиш операций (+,-,×,÷,обр.дел), которые ВЗВОДЯТ автоподстановку
  // (см. setOperator), и клавиши, которые её ПОТРЕБЛЯЮТ - те же пять
  // операций (если нажаты повторно без ввода нового значения), а также
  // степень, корень, закрывающая скобка, "=" и выделение целой части.
  // Любая клавиша НЕ из этого списка (цифры, точка, СК, смена знака, "(",
  // "Печать", регистровые кнопки) взвод снимает, ничего не подставляя -
  // см. общую проверку в конце handleKey/handleMemoryKey.
  ...
  const state = {
    ...
    // true, пока для уже выбранного оператора +,-,×,÷,обр.дел не появилось
    // НИКАКОГО нового значения - именно в этом случае действует
    // автоподстановка из A1 на месте пропущенного операнда (см.
    // getCurrentTermValue, AUTO_SUBSTITUTION_KEYS). "Взводится" только
    // выбором одного из этих пяти операторов (см. setOperator) и
    // "потребляется" (используется как признак подстановки) только при
    // нажатии одной из клавиш AUTO_SUBSTITUTION_KEYS - +,-,×,÷,обр.дел,
    // **, √, ")", "=", ВЦ; любая ДРУГАЯ клавиша (включая "(", "/-/", "()",
    // цифры/точку, регистровые кнопки, СК) этот "взвод" снимает, ничего не
    // подставляя - см. общую проверку в конце handleKey/handleMemoryKey.
    ...
    // Режим работы кнопок регистров A1-A5 (см. pressRegisterButton):
    // 'store'  - следующее нажатие A1..A5 ЗАШЛЁТ текущее значение в регистр
    //            (с перезаписью его прежнего содержимого);
    // 'recall' - следующее нажатие A1..A5 ВЫТАЩИТ значение регистра на экран
    //            (с заменой содержимого регистра на пустое/"нулевое").
    // Переключается строго по тому, какое действие предшествовало нажатию -
    // см. полный список триггеров в doc-комментарии вверху файла. Операция
    // "(" на этот режим не влияет (сознательно нигде не трогает
    // registerMode); "=" и ")" в списке триггеров тоже не упомянуты и,
    // соответственно, режим не меняют.
    registerMode: 'recall',
  ...
  function resetState() {
    ...
    state.registerMode = 'recall'; // СК - один из триггеров режима "выборка" для кнопок A1-A5 (см. doc-комментарий вверху файла)
  ...
  function handleKey(value) {
    ...
    // Любая клавиша, НЕ входящая в AUTO_SUBSTITUTION_KEYS, снимает "взвод"
    // автоподстановки А1 на месте пропущенного операнда - даже если её
    // собственный обработчик сам этот флаг не трогает (например "(" или
    // "/-/"). Клавиши ИЗ списка сами корректно управляют operandPending
    // через getCurrentTermValue/finalizeContextTerm/ensureCurrentValueIsTermValue,
    // трогать их здесь не нужно (для +,-,×,÷,обр.дел setOperator сам
    // заново "взводит" флаг для следующего терма - см. ниже).
  ...
  function handleMemoryKey(value) {
    ...
    resetRootChain();
    // Регистровые кнопки (A1-A5, "2", "3") не входят в AUTO_SUBSTITUTION_KEYS -
    // тоже снимают "взвод" автоподстановки А1 (см. handleKey)
    state.operandPending = false;
  ...
  // независимые операции "с перезаписью", а выбор между ними определяется
  // ДЕЙСТВИЕМ, которое предшествовало нажатию (state.registerMode - см. её
  // doc-комментарий в объявлении state и полный список триггеров в
  // doc-комментарии вверху файла), а не тем, что сейчас на экране:
  //  - registerMode === 'store' (перед этим было: ввод числа, степень,
  //    корень, вычисление целой части, отрицание) -> "заслать": текущее
  //    значение ВСЕГДА (даже если в регистре уже что-то лежало) записывается
  //    в регистр, замещая его прежнее содержимое. Старое содержимое регистра
  //    нигде не сохраняется и на экран не возвращается;
  //  - registerMode === 'recall' (перед этим было любое из: +,-,×,÷,обр.дел,
  //    накопление в A2/A3, СК, другая регистровая кнопка, вывод буфера
  //    клавиатуры) -> "вытащить": значение регистра выводится на экран, а
  //    сам регистр заменяется нулём/пустотой (как будто в него никогда не
  //    клали значение). Если регистр и так пуст - вытаскивать нечего,
  ...
  function pressRegisterButton(name) {
    ...
    // Само нажатие A1..A5 - триггер режима "выборка" для СЛЕДУЮЩЕГО нажатия
    // любой регистровой кнопки (см. doc-комментарий вверху файла) - вне
    // зависимости от того, засылка это была или выборка
    state.registerMode = 'recall';
    renderRegisters();
  }
  ...
  // (√, ** или ВЦ): убеждается, что на экране лежит именно ТЕКУЩИЙ терм - то,
  ...
  // которая всегда сворачивала весь контекст целиком. Так корень/степень/ВЦ
  ...
  // сразу после выбора оператора нажали √/**/ВЦ) - подставляем то же
  ...
  function openParenthesis() {
    ...
    state.hasValue = false;
    state.waitingForNewEntry = false;
    // "(" не входит в AUTO_SUBSTITUTION_KEYS: открытие скобки не "взводит"
    // автоподстановку А1 - новый (вложенный) контекст должен получить
    // реально введённое значение, а не молча подставленное А1
    state.operandPending = false;
    renderDisplay();
  }
  ...
  function setOperator(operator) {
    ...
    // Терм всегда фиксируется здесь - в т.ч. если предыдущий оператор ещё
    // ждал операнд (ctx.pendingOperator !== null && operandPending): по
    // новому правилу повторное нажатие любой из клавиш +,-,×,÷,обр.дел без
    // ввода нового значения тоже включено в автоподстановку А1 - работает
    // точно так же, как ")", "=", "√", "**", "ВЦ" (см. AUTO_SUBSTITUTION_KEYS,
    // getCurrentTermValue). Заодно замораживает регистр клавиатуры (см.
    // finalizeContextTerm).
    finalizeContextTerm(ctx);
    ...
    state.hasValue = true;
    state.registerMode = 'recall'; // +,-,×,÷,обр.дел - триггер режима "выборка" для A1-A5 (см. doc-комментарий вверху файла)
    renderDisplay();
  ...
  function integerPart() {
    if (state.overflow) return;
    state.registerMode = 'store'; // ВЦ - триггер режима "заслать" для A1-A5 (см. doc-комментарий вверху файла)
    // ВЦ входит в AUTO_SUBSTITUTION_KEYS - если перед этим был выбран
    // оператор без последующего ввода значения, недостающий операнд
    // подставляется из A1 точно так же, как для "=", ")", "√", "**"
    ensureCurrentValueIsTermValue();
